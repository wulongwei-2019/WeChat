package wechat.wechatApi.util.http.Impl;
...
import wechat.wechatApi.util.http.AbstractHttpClient;
import wechat.wechatApi.util.http.HttpClientBuilder;
import wechat.wechatApi.util.http.HttpDelegate;
...
public class HttpClientDelegate extends AbstractHttpClient implements HttpDelegate{
	...
	public String post(String url, Map<String, String> params, String postData) {
		...
		try {
			...
			LOGGER.error(java.text.Normalizer.normalize(
					String.format("Post error.url: %s , params: %s , postData: ", url, params, postData),
					java.text.Normalizer.Form.NFKD), e);
		}
		return response;
	}
	...
	public byte[] readURLImage(String imageUrl) throws IOException {
		...
		try (InputStream inStream = conn.getInputStream();) {
			...
			//if (ImageUtils.isImage(inStream)) {
